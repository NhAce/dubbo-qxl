...
public class UrlUtils {
    ...
    /**
     * 根据 <dubbo:registry> 中的 address 和对应的配置参数，装配 URL 对象
     * @param address 当前 <dubbo:registry> 的 address 属性
     * @param defaults 配置参数
     * @return
     */
    public static URL parseURL(String address, Map<String, String> defaults) {
        ...
        String url;
        // 如果 address 的值包含了 :// 或 ? ，则把 address 的值赋给 url
        ...
        //以当前配置信息为准，复制一个新的 map
        Map<String, String> defaultParameters = defaults == null ? null : new HashMap<String, String>(defaults);
        //主要把协议类型，端口，path 属性移除
        ...
        //根据 url 字符串，构造一个 url 对象
        URL u = URL.valueOf(url);
        //改变标志设为 false
        boolean changed = false;
        //从生成的 url 对象中获取 protocol, username, password, host, port, path 等属性
        ...
        //复制 url 对象中的 parameters 数据结构
        Map<String, String> parameters = new HashMap<String, String>(u.getParameters());
        //protocol, username, password, host, port, path 这几个属性，如果 url 中 get 到的为空，且默认的 defaults 配置不为空
        //则把 defaults 中的值赋给 url 中 get 到的值，且把改变标志 changed 设为 true
        ...
        //当且仅当 defaults 中的key的值不为 null 且 parameter 中对应的 key 的值为 null 时，为 url 中 parameters 赋值，
        ...
        //一旦 protocol, username, password, host, port, path，parameters 这些属性中任一一个发生了改变，根据最新的值，重新构造一个 url 对象出来，并返回
        if (changed) {
