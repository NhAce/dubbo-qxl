...
public abstract class AbstractInterfaceConfig extends AbstractMethodConfig {
    ...
    protected List<URL> loadRegistries(boolean provider) {
        ...
        // 配置文件中配置了 <dubbo:registry>
        if (CollectionUtils.isNotEmpty(registries)) {
            //当配置文件中配置了多个 <dubbo:registry> 标签时遍历
            for (RegistryConfig config : registries) {
                //获取当前 <dubbo:registry> 中对应的 address 属性
                ...
                if (!RegistryConfig.NO_AVAILABLE.equalsIgnoreCase(address)) {
                    ...
                    //为 map 赋值，<dubbo:application> 中配的值和默认值
                    appendParameters(map, application);
                    //为 map 赋值，当前 <dubbo:registry> 中配的值和默认值
                    appendParameters(map, config);
                    //保存 RegistryService 的全类名 org.apache.dubbo.registry.RegistryService
                    map.put(PATH_KEY, RegistryService.class.getName());
                    //装载运行时参数，包括 dubbo 版本，时间戳，release 版本，pid 等
                    appendRuntimeParameters(map);
                    //如果以上配置没有解析出协议类型，则默认使用 dubbo 协议
                    if (!map.containsKey(PROTOCOL_KEY)) {
                        map.put(PROTOCOL_KEY, DUBBO_PROTOCOL);
                    }
                    //因为 address 字符串中可以填写多个地址，所以用 list 接收对应的 url 对象
                    List<URL> urls = UrlUtils.parseURLs(address, map);

                    for (URL url : urls) {
                        //把 url 中原本的 protocol 属性设为 registry，并且在 parameters 属性中新增了一个 key 为 registry 的属性，并把原本 protocol 属性的值作为它的 value
                        //为 Dubbo SPI 准备
                        url = URLBuilder.from(url)
                                .addParameter(REGISTRY_KEY, url.getProtocol())
                                .setProtocol(REGISTRY_PROTOCOL)
                                .build();
                        //如果是服务提供者，判断是否只订阅不注册。如果是，不添加结果到 registryList 中。
                        //如果是服务消费者，判断是否只注册不订阅。如果是，不添加结果到 registryList 中。
