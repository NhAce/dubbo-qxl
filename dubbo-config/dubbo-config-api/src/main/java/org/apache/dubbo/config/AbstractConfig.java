        ...
        for (Method method : methods) {
            try {
                ...
                if (MethodUtils.isGetter(method)) {
                    ...
                    //如果方法返回值是 object 或者 注解不为 null 且注解的 excluded 属性为 true，直接 pass 到下一个方法
                    if (method.getReturnType() == Object.class || parameter != null && parameter.excluded()) {
                        continue;
                    }
                    String key;
                    //注解不为空且注解上的 key 属性不为空，用注解上 key 的值作为 key
                    if (parameter != null && parameter.key().length() > 0) {
                        key = parameter.key();
                    } else {
                        //否则，取 get 或 is 后面的值作为 key，所有大写字母会改成小写
                        key = calculatePropertyFromGetter(name);
                    }
                    //调用 config 的 method 方法，获得 value
                    Object value = method.invoke(config);
                    //value 转化为字符串
                    String str = String.valueOf(value).trim();
                    //value 不为null，即用户配置了该方法所获取的属性的值
                    if (value != null && str.length() > 0) {
                        //如果注解不为null且注解的 escaped 属性是 true
                        if (parameter != null && parameter.escaped()) {
                            //对字符串结果进行编码
                            str = URL.encode(str);
                        }
                        //如果注解不为null且注解的 append 属性是 true
                        if (parameter != null && parameter.append()) {
                            //从配置参数中获取 default.key 的值
                            String pre = parameters.get(DEFAULT_KEY + "." + key);
                            if (pre != null && pre.length() > 0) {
                                //配置参数中 default.key 的值不为空，把 pre 加到 str 前，并用 , 分割
                                str = pre + "," + str;
                            }
                            //从配置参数中获取 key 的值
                            pre = parameters.get(key);
                            if (pre != null && pre.length() > 0) {
                                //配置参数中获取 default.key 的值
                                str = pre + "," + str;
                            }
                        }
                    ...
                    //如果方法名为 getParameters 且方法修饰符是 public 且 参数列表长度为 0 且 返回类型是 map
                    //调用 config 的 method 方法，
