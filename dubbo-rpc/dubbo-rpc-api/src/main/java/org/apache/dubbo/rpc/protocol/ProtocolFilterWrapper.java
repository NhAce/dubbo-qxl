...
public class ProtocolFilterWrapper implements Protocol {
     ...
     * @param invoker 传入的 invoker 参数
     * @param key 通过 key 获取 invoker 中 url 的参数属性
     * @param group 分组
     * @param <T>
     * @return
     */
    private static <T> Invoker<T> buildInvokerChain(final Invoker<T> invoker, String key, String group) {
        ...
        if (!filters.isEmpty()) {
            ...
            /**
             * 假设 filters 的顺序为 filter1, filter2, filter3
             * 一下循环结束后，last 的结构可以表示为：
             * last = invoke(filter1(invoke(filter2(invoke(filter3(invoke))))))
             */
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        // invoker 中 url 的 protocol 属性等于 registry，直接暴露
    ...
    public <T> Invoker<T> refer(Class<T> type, URL url) throws RpcException {
        // invoker 中 url 的 protocol 属性等于 registry，直接引用
        if (REGISTRY_PROTOCOL.equals(url.getProtocol())) {
            return protocol.refer(type, url);
        }
        // 引用服务，返回 Invoker 对象
        // 将该 Invoker 对象，包装成带有 Filter 过滤链的 Invoker 对象
